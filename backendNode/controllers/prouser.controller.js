...
ProUserController.getProfile = async (req) => {
...
ProUserController.signup = async (req, res, next) => {
  ...
  try {
    ...
    return next(InternalServerError('Server error || duplicate data || wrong code'));
  }
};
...
try {
  const backendResponse = await fetch(`http://10.184.19.231:9001/predict/composite`, {
    method: "POST",
    headers: {
      'Content-Type': 'application/json',
      'Authorization': process.env.FLASK_SECRET_CODE,
